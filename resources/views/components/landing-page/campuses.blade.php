...
<section id="campuses" class="py-20 bg-white">
    <div class="container px-4 mx-auto">
        <div class="text-center mb-16">
            <h2 class="text-3xl md:text-4xl font-bold text-gray-800 mb-4">Our Campuses</h2>
            ...
            @forelse($campuses as $campus)
                <div class="group relative bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2">
                    ...
                    <div class="relative h-64 overflow-hidden bg-gray-100">
                        @if($campus->image)
                            <img src="{{ asset($campus->image->file_dir) }}"
                                 alt="{{ $campus->name }}"
                                 class="w-full h-full object-cover transform transition-transform duration-500 group-hover:scale-110"
                                 onerror="this.src='https://images.unsplash.com/photo-1562774053-701939374585?w=800&auto=format&fit=crop'">
                        @else
                            <div class="w-full h-full flex items-center justify-center bg-gradient-to-br from-accent/20 to-accent/10">
                        ...
                        <div class="absolute bottom-0 left-0 right-0 p-4 text-white transform transition-transform duration-300">
                            <h3 class="text-xl font-bold mb-1 line-clamp-2">{{ $campus->name }}</h3>
                        </div>

                        <!-- Hover Details Overlay -->
                        <div class="absolute inset-0 bg-accent/95 p-6 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-center">
                            <div class="text-white space-y-4">
                                <div>
                                    <h3 class="text-xl font-bold mb-3 border-b border-white/30 pb-2">{{ $campus->name }}</h3>
                                </div>
