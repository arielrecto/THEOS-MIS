                        ...
                        <div class="flex items-start gap-3 p-4">
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex items-start justify-between gap-3">
                                    <div class="min-w-0">
                                        <p class="text-sm sm:text-base font-medium text-gray-800 truncate">{{ $classroom->name }}</p>
                                        <p class="text-xs sm:text-sm text-gray-500 mt-0.5 truncate">{{ $classroom->section ?? $classroom->subject->name ?? 'N/A' }}</p>
                                        <p class="text-xs text-gray-400 mt-1">{{ $classroom->updated_at->format('M d, Y') }}</p>
                                    </div>

                                    <div class="text-right flex-shrink-0">
                                        <p class="text-xs text-gray-500">{{ $classroom?->classroomStudents()?->count() ?? 0 }}</p>
                                    </div>
                                </div>
                                ...
                                <div class="mt-3 flex items-center gap-2">
                                    ...
                                    <form id="unarchive-form-{{ $classroom->id }}" action="{{ route('teacher.classrooms.unarchive', $classroom) }}"
                                    ...
                                    <a href="{{ route('teacher.classrooms.show', $classroom) }}" class="ml-auto text-xs text-accent font-medium">
                                        View
                                    </a>
                                </div>
                            </div>
                                ...
                                <tr>
                                    <td>
                                        <div class="flex flex-col min-w-0">
                                            ...
                                            <span class="text-xs text-gray-500 truncate">{{ $classroom->section }}</span>
                                        </div>
                                    </td>
    ...
    <dialog id="delete_modal" class="modal">
        <div class="modal-box">
            ...
            <p class="py-4 flex items-start gap-3">
                ...
                <span class="text-sm">Are you sure you want to permanently delete this classroom? This action cannot be undone.</span>
            </p>
