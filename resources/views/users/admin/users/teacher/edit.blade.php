<x-dashboard.admin.base>
    ...
    <div class="panel flex flex-col gap-2">
        <form action="{{route('admin.users.teacher.update', ['teacher' => $teacher->id])}}" method="post" class="w-full flex flex-col gap-2 capitalize">
            @csrf
            @method('put')
            <h1 class="form-title">
                Teacher Edit Form
            ...
            <div class="flex flex-col gap-2">
                ...
                <input type="text" name="name" class="input-generic" placeholder="{{$teacher->name}}">
                @if ($errors->has('name'))
            ...
            <div class="flex flex-col gap-2">
                ...
                <input type="email" name="email" class="input-generic" placeholder="{{$teacher->email}}">
                @if ($errors->has('email'))
                    <p class="text-xs text-error">
                        {{ $errors->first('email') }}
                    </p>
