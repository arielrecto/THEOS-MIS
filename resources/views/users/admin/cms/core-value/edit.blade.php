    ...
    <div class="container mx-auto p-4 sm:p-6 lg:p-8 max-w-5xl">
        ...
        <div class="mb-6">
            <div class="flex items-center gap-3 mb-2">
                ...
                <h1 class="text-2xl sm:text-3xl font-bold text-gray-800">Edit Core Value</h1>
            </div>
            <p class="text-sm text-gray-600 ml-12">Update core value and items</p>
        </div>

        <div class="bg-white rounded-lg shadow-sm p-4 sm:p-6">
            <form action="{{ route('admin.CMS.core-values.update', $coreValue) }}" method="POST" class="space-y-6"
                x-data="coreValueForm(JSON.stringify({{ json_encode($coreValue->items) }}))">
                @csrf
                @method('PUT')
                ...
                <div class="form-control w-full">
                    ...
                    <textarea name="description" rows="4"
                        ...
                        placeholder="Enter core value description" required>{{ old('description', $coreValue->description) }}</textarea>
                    @error('description')
                        <label class="label">
                            <span class="label-text-alt text-error">{{ $message }}</span>
                        </label>
