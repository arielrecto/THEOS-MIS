<x-dashboard.student.base>
    <div class="container max-w-7xl mx-auto px-4 sm:px-6 p-6">
        <h1 class="mb-6 text-2xl font-bold text-gray-700">Account Settings</h1>

        @if ($errors->any())
            <div class="mb-4 space-y-1 text-sm text-red-600">
                @foreach ($errors->all() as $error)
                    <p class="truncate break-words">{{ $error }}</p>
                @endforeach
            </div>
        @endif
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div class="p-6 bg-white rounded-lg shadow-md">
                ...
                <form method="POST" action="{{ route('student.settings.profile.update') }}" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div class="min-w-0">
                            ...
                            <input type="text" disabled name="lrn"
                                ...
                                class="mt-1 w-full input input-bordered truncate" required>
                            @error('lrn')
                                <p class="mt-1 text-sm text-error break-words">{{ $message }}</p>
                            @enderror
                        </div>

                        <div class="min-w-0">
                            <label class="block text-sm font-medium text-gray-700">Contact Number</label>
                            <input type="text" name="contact_number" inputmode="numeric" minlength="11"
                        ...
                        <div class="min-w-0">
                            ...
                            <input type="text" name="parent_name"
                                ...
                                class="mt-1 w-full input input-bordered break-words" required>
                            @error('parent_name')
                                <p class="mt-1 text-sm text-error break-words">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        {{-- Example full-width field for address --}}
                    ...
                    <div class="flex justify-end">
                        <button type="submit" class="btn btn-accent w-full sm:w-auto">Update Profile</button>
                    </div>
                </form>
            </div>
            ...
            <div class="p-6 bg-white rounded-lg shadow-md">
                ...
                <form method="POST" action="{{ route('student.settings.email.update') }}" class="space-y-6">
                    ...
                    <div class="flex justify-end">
                        <button type="submit" class="btn btn-accent w-full sm:w-auto">Update Email</button>
                    </div>
                </form>
            </div>

            <!-- Update Password -->
            <div class="p-6 bg-white rounded-lg shadow-md sm:col-span-2">
                ...
                <form method="POST" action="{{ route('student.settings.password.update') }}" class="space-y-6">
                    ...
                    <div class="flex justify-end">
                        <button type="submit" class="btn btn-accent w-full sm:w-auto">Update Password</button>
                    </div>
                </form>
            </div>
        </div>
